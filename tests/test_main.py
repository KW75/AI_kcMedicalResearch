...
from SOURCE_CODE.main import (
    ...
    ALL_MODES, VERSION, COLOURS, main,
    generate_writing_report, generate_code_revision,
    run_rct_search_pipeline, _read_code_files, _read_article_files,
    _read_topic_file, save_rct_search_links, fetch_pubmed_articles,
    rct_search_reminder,
)
...
def test_build_project_context_includes_file_content(tmp_path: Path) -> None:
    ...
    with patch.dict("SOURCE_CODE.main.DOC_FILES_BY_ROLE", {"Builder": [doc]}):
...
def test_build_project_context_skips_missing_files(tmp_path: Path) -> None:
    ...
    with patch.dict("SOURCE_CODE.main.DOC_FILES_BY_ROLE", {"Builder": [missing]}):
...
def test_build_project_context_combines_multiple_files(tmp_path: Path) -> None:
    ...
    with patch.dict("SOURCE_CODE.main.DOC_FILES_BY_ROLE", {"Builder": [a, b]}):
...
def test_build_project_context_builder_receives_coding_standards(tmp_path: Path) -> None:
    ...
    with patch.dict("SOURCE_CODE.main.DOC_FILES_BY_ROLE", {"Builder": patched}):
...
def test_build_project_context_reviewer_receives_decision_log(tmp_path: Path) -> None:
    ...
    with patch.dict("SOURCE_CODE.main.DOC_FILES_BY_ROLE", {"Reviewer": patched}):
...
def test_build_project_context_tester_receives_test_strategy(tmp_path: Path) -> None:
    ...
    with patch.dict("SOURCE_CODE.main.DOC_FILES_BY_ROLE", {"Tester": patched}):
...
def test_call_ollama_provider_returns_response_text() -> None:
    ...
    with patch("SOURCE_CODE.main.urlopen", return_value=_mock_urlopen(body)):
...
def test_call_ollama_provider_raises_on_http_error() -> None:
    with patch("SOURCE_CODE.main.urlopen", side_effect=HTTPError(None, 500, "Server Error", {}, None)):
        with pytest.raises(RuntimeError, match="HTTP error"):
            call_ollama_provider("prompt")


def test_call_ollama_provider_raises_on_url_error() -> None:
    with patch("SOURCE_CODE.main.urlopen", side_effect=URLError("connection refused")):
...
def test_call_ollama_provider_raises_on_timeout() -> None:
    import socket
    with patch("SOURCE_CODE.main.urlopen", side_effect=TimeoutError("timed out")):
...
def test_main_dry_run_does_not_call_ai(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("SOURCE_CODE.main.REPORTS_DIR", tmp_path)
    with patch("SOURCE_CODE.main.call_ai") as mock_ai:
        try:
            main(dry_run=True)
...
def test_main_runs_full_workflow(tmp_path: Path) -> None:
    ...
    with patch("builtins.input", side_effect=["1", "Write a hello world function.", KeyboardInterrupt()]), \
         patch("SOURCE_CODE.main.urlopen", return_value=_mock_urlopen(body)), \
         patch("SOURCE_CODE.main.REPORTS_DIR", tmp_path), \
         patch("builtins.print"):
...
def test_main_uses_model_override(tmp_path: Path) -> None:
    body = json.dumps({"response": "AI response."}).encode()
    with patch("builtins.input", side_effect=["1", "Write a function.", KeyboardInterrupt()]), \
         patch("SOURCE_CODE.main.urlopen", return_value=_mock_urlopen(body)) as mock_u, \
         patch("SOURCE_CODE.main.REPORTS_DIR", tmp_path), \
         patch("builtins.print"):
...
def test_main_uses_env_model_when_no_override(tmp_path: Path) -> None:
    ...
    with patch("builtins.input", side_effect=["1", "Write a function.", KeyboardInterrupt()]), \
         ...
         patch("SOURCE_CODE.main.REPORTS_DIR", tmp_path), \
         patch("builtins.print"), \
         patch("SOURCE_CODE.main.OLLAMA_MODEL", "mistral:7b"):
        main(mode="coding", provider="ollama")
    call_data = json.loads(mock_u.call_args[0][0].data)
...
def test_main_handles_ollama_error_and_retries(tmp_path: Path) -> None:
    body = json.dumps({"response": "AI response."}).encode()
    with patch("builtins.input", side_effect=["1", "Write a function", KeyboardInterrupt()]), \
         patch("SOURCE_CODE.main.urlopen", side_effect=URLError("refused")), \
         patch("SOURCE_CODE.main.REPORTS_DIR", tmp_path), \
         patch("builtins.print"):
        main(mode="coding", provider="ollama")
