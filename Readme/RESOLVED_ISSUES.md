...
| 67 | Ang 2010 reading unverified | Session 27 - verified against PDF Table 2, page 621 (change in NFR pain rating from baseline to week 6). Ang stable across N=3 (nondet_flag=[]); outcome=NFR pain rating week 6; values -20.2 (23.9) CBT vs -14.9 (16.4) UC. Same numbers appear in the abstract and Discussion. S26 handoff's "table_shift on FIQ" framing was superseded by pipeline changes before verification. No study_overrides.yaml entry needed. |
...
| 69 | Text-fallback path drops studies for missing Ns silently | Session 27 - Stage 4 `[SKIP]` summary block in `_log_stage4_summary` names any study dropped from the pooled estimate; missing-per-arm-N drops (the Lami-shaped failure typical of text_fallback) are separated from other skip reasons and point the reviewer at `study_overrides.yaml`. Automated N-recovery from text-fallback source quotes is out of scope: the fallback returns means/SDs from one timepoint's quote but the paper often reports Ns at multiple timepoints (Lami: 28/36 post vs 34/41 baseline), and a silent wrong N is worse than a missing one. Pinned by `test_stage4_skip_missing_n.py` (3 tests). |
